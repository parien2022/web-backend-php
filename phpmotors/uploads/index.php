...
session_start();
//This is the upload images controller
...
switch ($action) {
    ...
    case 'upload':
        ...
        if($imageCheck){
            $message = "<p class='errMessage'>An image by that name already exists.</p>";
        } elseif (empty($invId) || empty($imgName)) {
            $message = "<p class='errMessage'>You must select a vehicle and image file for the vehicle.</p>";
        } else{
        ...
        if($result){
            $message = "<p class='sucMessage'>The upload succeeded</p>";
        } else{
            $message = "<p class='errMessage'>Sorry, the upload failed</p>";
        }

        echo "This is the current message: " . $message;
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p class='sucMessage'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p class='errMessage'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = "<p class='errMessage'>Sorry, no images could be found.</p>";
        }
